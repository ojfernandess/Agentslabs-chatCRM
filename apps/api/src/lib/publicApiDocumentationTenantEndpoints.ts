...
export const PUBLIC_TENANT_API_DOCUMENTATION_ENDPOINTS: PublicApiDocEndpoint[] = [
  ...
  },
  {
    ...
    descriptionEn:
      "Send outbound messages: TEXT (WhatsApp 24h window), media (IMAGE/VIDEO/DOCUMENT via mediaUrl), email replies, internal notes (isPrivate), or TEMPLATE (WhatsApp Business with templateBodyParameters). Obtain templateId from GET /api/v1/templates.",
    descriptionPt:
      "Enviar mensagens de saída: TEXT (janela 24h WhatsApp), mídia (IMAGE/VIDEO/DOCUMENT via mediaUrl), respostas de e-mail, notas internas (isPrivate) ou TEMPLATE (WhatsApp Business com templateBodyParameters). Obtenha templateId em GET /api/v1/templates.",
    examplePayloadPt:
      'Autenticação: Authorization: Bearer <jwt> OU api_access_token: ocu_… / Bearer ocu_…\n\nPOST /api/v1/messages\nContent-Type: application/json\n\n— Send Message (texto WhatsApp):\n{\n  "contactId": "<uuid-contacto>",\n  "conversationId": "<uuid-conversa-opcional>",\n  "inboxId": "<uuid-caixa-whatsapp-opcional>",\n  "type": "TEXT",\n  "body": "Olá! Recebemos o seu pedido e já estamos a tratar.",\n  "isPrivate": false\n}\n\nNotas:\n• contactId é obrigatório.\n• isPrivate: false envia ao cliente; true grava só nota interna.\n• Fora da janela 24h no WhatsApp Cloud/Meta, texto livre falha — use TEMPLATE.\n\n— Send Template:\n{\n  "contactId": "<uuid-contacto>",\n  "type": "TEMPLATE",\n  "templateId": "<uuid-modelo-sincronizado>",\n  "templateBodyParameters": ["Maria Silva", "Pedido #4521", "12/08/2026"]\n}\n\nRegras TEMPLATE:\n• templateBodyParameters na ordem de {{1}}, {{2}}, … — contagem = bodyVariableCount do modelo.\n• isPrivate não é permitido com TEMPLATE.\n• Modelo precisa de providerTemplateId (sincronize via GET/POST /api/v1/templates).\n\n— curl (texto):\ncurl -X POST "https://<seu-dominio>/api/v1/messages" \\\n  -H "Authorization: Bearer ocu_<token-perfil>" \\\n  -H "Content-Type: application/json" \\\n  -d \'{"contactId":"<uuid>","type":"TEXT","body":"Olá!","isPrivate":false}\'',
  },
  {
    method: "POST",
