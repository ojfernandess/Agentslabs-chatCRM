...
import {
  ...
  PAYMENT_CONFIRMATION_PREVIEW_SAMPLE,
...
export type ResendPaymentConfirmationTemplateEditorProps = {
  fromName: string;
  logoUrl?: string;
...
export function ResendPaymentConfirmationTemplateEditor({
  fromName,
  logoUrl,
  resolvedLogoUrl,
  subject,
  ...
  onSubjectChange,
  onHtmlChange,
}: ResendPaymentConfirmationTemplateEditorProps) {
  const { t } = useI18n();
  ...
  const preview = useMemo(() => {
    ...
    return buildPaymentConfirmationEmailContent(subject, html, {
      ...PAYMENT_CONFIRMATION_PREVIEW_SAMPLE,
      appName,
    ...
    <div className="space-y-3 border-t border-ink-100 pt-4 dark:border-ink-700">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            onSubjectChange(DEFAULT_PAYMENT_CONFIRMATION_SUBJECT);
            onHtmlChange(DEFAULT_PAYMENT_CONFIRMATION_HTML);
          }}
        >
      ...
      {tab === "edit" ? (
        <>
          <div>
            <h3 className="mb-1 text-sm font-semibold text-ink-900">{t("superAdmin.resendPaymentConfirmationSubject")}</h3>
            <p className="mb-2 text-xs text-ink-500">{t("superAdmin.resendPaymentConfirmationPlaceholders")}</p>
            <input
              ref={subjectInputRef}
              type="text"
              value={subject}
          ...
          <div>
            <h3 className="mb-1 text-sm font-semibold text-ink-900">{t("superAdmin.resendPaymentConfirmationHtml")}</h3>
            <textarea
              ref={htmlTextareaRef}
              value={html}
