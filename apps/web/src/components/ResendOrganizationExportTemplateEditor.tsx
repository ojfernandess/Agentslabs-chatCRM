...
import {
  buildOrganizationExportEmailContent,
  DEFAULT_ORGANIZATION_EXPORT_HTML,
  DEFAULT_ORGANIZATION_EXPORT_SUBJECT,
  ORGANIZATION_EXPORT_PREVIEW_SAMPLE,
...
export type ResendOrganizationExportTemplateEditorProps = {
  fromName: string;
  logoUrl?: string;
...
export function ResendOrganizationExportTemplateEditor({
  fromName,
  logoUrl,
  resolvedLogoUrl,
  subject,
  ...
  onSubjectChange,
  onHtmlChange,
}: ResendOrganizationExportTemplateEditorProps) {
  const { t } = useI18n();
  ...
  const preview = useMemo(() => {
    ...
    return buildOrganizationExportEmailContent(subject, html, {
      ...ORGANIZATION_EXPORT_PREVIEW_SAMPLE,
      appName,
    ...
    <div className="space-y-3 border-t border-ink-100 pt-4 dark:border-ink-700">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            onSubjectChange(DEFAULT_ORGANIZATION_EXPORT_SUBJECT);
            onHtmlChange(DEFAULT_ORGANIZATION_EXPORT_HTML);
          }}
        >
      ...
      {tab === "edit" ? (
        <>
          <div>
            <h3 className="mb-1 text-sm font-semibold text-ink-900">{t("superAdmin.resendOrganizationExportSubject")}</h3>
            <p className="mb-2 text-xs text-ink-500">{t("superAdmin.resendOrganizationExportPlaceholders")}</p>
            <input
              ref={subjectInputRef}
              type="text"
              value={subject}
          ...
          <div>
            <h3 className="mb-1 text-sm font-semibold text-ink-900">{t("superAdmin.resendOrganizationExportHtml")}</h3>
            <textarea
              ref={htmlTextareaRef}
              value={html}
