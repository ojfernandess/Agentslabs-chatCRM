...
type TelephonyProvider = "wavoip" | "nvoip";
...
function useTelephonyProviders() {
  const { user } = useAuth();
  ...
  return { providers, wavoipVoice, nvoipVoice, wavoipCan, nvoipCan };
}

export function TelephonyDialModal({ open, onClose }: Props) {
  const { t } = useI18n();
  const { providers, wavoipVoice, nvoipVoice } = useTelephonyProviders();
  const [provider, setProvider] = useState<TelephonyProvider | null>(null);
  ...
  const activeProvider = provider && providers.includes(provider) ? provider : providers[0] ?? null;

  useEffect(() => {
    if (!open) return;
    setProvider((prev) => (prev && providers.includes(prev) ? prev : providers[0] ?? null));
  }, [open, providers]);
  ...
  useEffect(() => {
    ...
    if (!isValidDialLength(raw) || !activeProvider) {
      setResolvedContext(null);
      return;
    }
    const resolvePath =
      activeProvider === "wavoip"
        ? `/wavoip/calls/resolve-context?phone=${encodeURIComponent(raw)}`
        : `/nvoip/calls/resolve-context?phone=${encodeURIComponent(raw)}`;
    const id = window.setTimeout(() => {
      void (async () => {
        setResolvingPhone(true);
        try {
          const ctx = await api.get<ResolveContext>(resolvePath);
  ...
  const hasActiveCall =
    (activeProvider === "wavoip" && !!wavoipVoice?.activeCall) ||
    (activeProvider === "nvoip" && !!nvoipVoice?.activeCall);

  const placeCall = async (input: { phone: string; contactId?: string | null }) => {
    if (!activeProvider) {
      setError(t("telephony.dial.noProviders"));
      return;
    }
        ...
        try {
          const resolvePath =
            activeProvider === "wavoip"
              ...
              : `/nvoip/calls/resolve-context?phone=${encodeURIComponent(phone)}`;
          const ctx = await api.get<ResolveContext>(resolvePath);
          if (ctx.contact) {
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {providers.length === 0 ? (
              <p className="px-4 py-10 text-center text-sm text-gray-500 dark:text-ink-400">
                        ...
                        <li key={c.id}>
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1">
                              ...
                              <span className="block truncate text-xs text-gray-500 dark:text-ink-400">
                                {c.phone}
                              </span>
                            </span>
                          </button>
...
export function useTelephonyCanDial(): boolean {
