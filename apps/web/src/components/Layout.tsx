  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex h-9 w-9 items-center justify-center rounded-lg border border-ink-100 bg-ink-50 text-ink-500 hover:bg-ink-100 dark:border-soft-border-muted dark:bg-soft-surface-2/60 dark:text-ink-300 dark:hover:bg-soft-surface-3"
      ...
      {open ? (
        <div
          className="absolute bottom-0 left-full z-[120] ml-2 min-w-[10.5rem] overflow-hidden rounded-lg border border-ink-200 bg-white py-1 shadow-lg dark:border-soft-border-muted dark:bg-soft-surface-2"
          role="menu"
        >
          {options.map((opt) => (
            <button
...
export function Layout() {
    ...
    <>
        ...
        <div className={clsx("flex w-full items-center", collapsed ? "justify-center" : "gap-1")}>
          <Link
            ...
            className={clsx(
              ...
              collapsed
                ? "justify-center rounded-xl bg-gradient-to-b from-brand-50/90 to-white p-2.5 shadow-sm ring-1 ring-brand-200/50 dark:from-brand-950/50 dark:to-soft-surface-2 dark:ring-brand-500/20"
      ...
      >
        ...
        {collapsed ? (
          ...
          <div className="flex w-full items-center gap-2 rounded-lg border border-ink-100 bg-ink-50 px-2 py-1.5 dark:border-soft-border-muted dark:bg-soft-surface-2/50">
  ...
  return (
    ...
    <div className="flex h-full min-h-0 w-full min-w-0 max-w-full overflow-x-clip">
      <aside
        className={clsx(
          "hidden shrink-0 flex-col border-r border-ink-200 bg-white transition-[width] duration-200 ease-in-out dark:border-soft-border-muted dark:bg-ink-950 lg:flex",
      ...
      {mobileNavOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden" role="dialog" aria-modal="true">
          ...
          />
          <aside className="relative flex h-full w-80 max-w-[85vw] shrink-0 flex-col border-r border-ink-200 bg-white shadow-xl dark:border-soft-border-muted dark:bg-ink-950">
