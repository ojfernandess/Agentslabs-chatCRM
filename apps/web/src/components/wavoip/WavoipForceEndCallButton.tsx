...
/** Desliga chamada ativa no SDK ou encerra estado «em ligação» persistido no servidor. */
export function WavoipForceEndCallButton({
  conversationId,
  activeVoiceCall,
  className,
  compact = false,
  ...
  if (!voice || activeVoiceCall?.provider !== "wavoip") return null;
  ...
  return (
    <button
      ...
      title={showStale ? t("wavoip.voice.forceEndCallHelp") : t("wavoip.voice.hangUp")}
      className={clsx(
    ...
    >
      ...
      <span>{t("wavoip.voice.forceEndCall")}</span>
    </button>
  );
}
