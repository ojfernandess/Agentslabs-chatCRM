...
import {
  BILLING_REMINDER_PREVIEW_SAMPLE,
...
export type ResendBillingReminderTemplateEditorProps = {
  fromName: string;
  logoUrl?: string;
...
export function ResendBillingReminderTemplateEditor({
  fromName,
  logoUrl,
  resolvedLogoUrl,
  subject,
  ...
  onSubjectChange,
  onHtmlChange,
}: ResendBillingReminderTemplateEditorProps) {
  const { t } = useI18n();
  ...
  const preview = useMemo(() => {
    ...
    return buildBillingReminderEmailContent(subject, html, {
      ...BILLING_REMINDER_PREVIEW_SAMPLE,
      appName,
    ...
    <div className="space-y-3 border-t border-ink-100 pt-4 dark:border-ink-700">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            onSubjectChange(DEFAULT_BILLING_REMINDER_SUBJECT);
            onHtmlChange(DEFAULT_BILLING_REMINDER_HTML);
          }}
        >
      ...
      {tab === "edit" ? (
        <>
          <div>
            <h3 className="mb-1 text-sm font-semibold text-ink-900">{t("superAdmin.resendBillingReminderSubject")}</h3>
            <p className="mb-2 text-xs text-ink-500">{t("superAdmin.resendBillingReminderPlaceholders")}</p>
            <input
              ref={subjectInputRef}
              type="text"
              value={subject}
          ...
          <div>
            <h3 className="mb-1 text-sm font-semibold text-ink-900">{t("superAdmin.resendBillingReminderHtml")}</h3>
            <textarea
              ref={htmlTextareaRef}
              value={html}
