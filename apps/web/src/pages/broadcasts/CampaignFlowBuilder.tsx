...
export function CampaignFlowBuilder({ value, onChange }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-dashed border-ink-200 bg-ink-50/50 p-3 dark:border-soft-border dark:bg-white/5">
        {flow.nodes.map((node) => (
          <div
            key={node.id}
            ...
            className={clsx(
              "mb-2 flex items-center gap-2 rounded-lg border bg-white px-3 py-2 text-xs shadow-sm dark:bg-[#202736]",
              dragId === node.id ? "border-brand-400" : "border-ink-200 dark:border-soft-border",
            )}
          >
            ...
            {node.type === "wait" ? (
              <input
                ...
                min={1}
                max={1440}
                className="w-14 rounded border border-ink-200 px-1 py-0.5 text-[10px] dark:border-soft-border dark:bg-white/5"
                placeholder="min"
      ...
      <div className="flex flex-wrap gap-1">
        {BLOCK_TYPES.filter((x) => x !== "start" && x !== "end").map((type) => (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg border border-ink-200 px-2 py-1 text-[10px] font-medium hover:bg-ink-50 dark:border-soft-border dark:hover:bg-white/5"
          >
