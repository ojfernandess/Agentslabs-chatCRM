...
const MP_WEBHOOKS_URL = "https://www.mercadopago.com.br/developers/panel/app";

const RECOMMENDED_EVENT_DESC_KEYS: Record<string, string> = {
  payment: "automationPage.toolMercadoPagoEventDesc_payment",
};

const ADDITIONAL_EVENT_DESC_KEYS: Record<string, string> = {
  subscription_preapproval: "automationPage.toolMercadoPagoEventDesc_subscription_preapproval",
  merchant_order: "automationPage.toolMercadoPagoEventDesc_merchant_order",
  subscription_authorized_payment: "automationPage.toolMercadoPagoEventDesc_subscription_authorized_payment",
};
...
export function MercadoPagoToolSetupHelp({
  t,
  webhookUrl,
  recommendedEvents,
  additionalEvents,
  ...
  const helpLabel = t("automationPage.toolMercadoPagoSetupHelpOpen");
      ...
      {open ? (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="mercadopago-tool-setup-help-title"
          ...
          >
            <div className="flex items-start justify-between gap-3 border-b border-ink-100 px-5 py-4 dark:border-ink-700">
              <h2
                id="mercadopago-tool-setup-help-title"
                className="text-sm font-semibold text-ink-900 dark:text-ink-50"
              >
                {t("automationPage.toolMercadoPagoSetupHelpTitle")}
              </h2>
              <button
                type="button"
                aria-label={t("automationPage.toolMercadoPagoSetupHelpClose")}
                onClick={() => setOpen(false)}
              ...
              <div className="space-y-5">
                <section>
                  <p className="text-[11px] leading-relaxed text-ink-600 dark:text-ink-400">
                    {t("automationPage.toolMercadoPagoSetupHelpIntro")}
                  </p>
                </section>

                <section>
                  <h3 className="text-xs font-semibold text-ink-800 dark:text-ink-100">
                    {t("automationPage.toolMercadoPagoSetupHelpStepsTitle")}
                  </h3>
                  <ol className="mt-2 list-decimal space-y-1.5 pl-4 text-[11px] leading-relaxed text-ink-600 dark:text-ink-400">
                    <li>{t("automationPage.toolMercadoPagoSetupHelpStep1")}</li>
                    <li>{t("automationPage.toolMercadoPagoSetupHelpStep2")}</li>
                    <li>
                      {t("automationPage.toolMercadoPagoSetupHelpStep3")}{" "}
                      <a
                        href={MP_WEBHOOKS_URL}
                        target="_blank"
                    ...
                    <li>{t("automationPage.toolMercadoPagoSetupHelpStep4")}</li>
                    <li>
                      {t("automationPage.toolMercadoPagoSetupHelpStep5")}
                      {webhookUrl ? (
                    ...
                    <li>{t("automationPage.toolMercadoPagoSetupHelpStep6")}</li>
                    <li>{t("automationPage.toolMercadoPagoSetupHelpStep7")}</li>
                    <li>{t("automationPage.toolMercadoPagoSetupHelpStep8")}</li>
                  </ol>
                </section>

                <section>
                  <h3 className="text-xs font-semibold text-ink-800 dark:text-ink-100">
                    {t("automationPage.toolMercadoPagoSetupHelpRecommendedTitle")}
                  </h3>
                  <p className="mt-1 text-[11px] leading-relaxed text-ink-600 dark:text-ink-400">
                    {t("automationPage.toolMercadoPagoSetupHelpRecommendedIntro")}
                  </p>
                ...
                <section>
                  <h3 className="text-xs font-semibold text-ink-800 dark:text-ink-100">
                    {t("automationPage.toolMercadoPagoSetupHelpAdditionalTitle")}
                  </h3>
                  <p className="mt-1 text-[11px] leading-relaxed text-ink-600 dark:text-ink-400">
                    {t("automationPage.toolMercadoPagoSetupHelpAdditionalIntro")}
                  </p>
