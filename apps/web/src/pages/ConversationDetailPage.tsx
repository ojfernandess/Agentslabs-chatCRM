...
export function ConversationDetailPage() {
  ...
  const submitTransfer = async () => {
    ...
    if (transferAssigneeId && !selectedTransferAssigneeOnline) return;
    setActionLoading(true);
    setFlowError("");
    try {
  ...
  if (!conversation || conversation.id !== id) {
    if (isEmbeddedLayout) {
      return (
        <div className="flex h-full min-w-0 flex-1 flex-col bg-ink-50 dark:bg-[#172033]">
          <div className="shrink-0 border-b border-ink-200 bg-white px-4 py-3 dark:border-slate-500/30 dark:bg-[#1c2a40]">
  ...
  const transferUnchanged = transferUsesTeams
    ...
    : (transferAssigneeId || null) === (conversation.assignedTo?.id ?? null);
  const selectedTransferAssignee = transferAssigneeId
    ...
    <div className="flex min-h-0 flex-col gap-0">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
          ...
          <Link
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-ink-200/80 bg-white text-ink-600 transition hover:bg-ink-50 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-300 dark:hover:bg-white/10"
        ...
        {opts?.showMobileClose ? (
          <button
            type="button"
            className="shrink-0 rounded-lg border border-ink-200/80 bg-white px-2.5 py-1.5 text-xs font-semibold text-ink-700 hover:bg-ink-50 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-200 dark:hover:bg-white/10"
      ...
      <div className="crm-card">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {(() => {
              ...
              const btnClass =
                "flex h-8 w-8 items-center justify-center rounded-lg border border-ink-200/80 bg-white text-ink-700 transition hover:bg-ink-50 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-100 dark:hover:bg-white/10";
        ...
        <div className="crm-panel-section">
          ...
          <div className="mt-2 max-h-52 overflow-y-auto rounded-lg border border-ink-100 bg-ink-50/70 px-2.5 py-2 dark:border-slate-500/15 dark:bg-white/5">
          ...
          <textarea
            ...
            className="mt-2 w-full resize-y rounded-lg border border-ink-200/80 bg-white px-2.5 py-2 text-sm text-ink-900 placeholder:text-ink-400 focus:border-brand-400/40 focus:outline-none focus:ring-1 focus:ring-brand-500/20 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-50 dark:placeholder:text-ink-500"
          />
          ...
          <div className="mt-1 divide-y divide-ink-100 border-t inbox-hairline-soft dark:divide-white/5">
            {conversation.closureRecords.map((rec) => {
              ...
              >
                ...
                <dl className="mb-2 space-y-1 border-l border-ink-200/80 pl-2 text-[10px] leading-snug text-ink-600 dark:border-slate-500/25 dark:text-ink-400">
                  {rec.resolvedBy ? (
                    <dd>
  ...
  return (
    <div
      className={clsx(
        ...
        emailWorkspaceMode
          ? "min-w-0 flex-1 flex-col bg-ink-50 dark:bg-[#172033] xl:flex-row"
          : "flex-col bg-ink-50 dark:bg-[#172033] lg:flex-row",
      )}
    >
      {!emailWorkspaceMode ? (
      ...
      void conversationsOutlet?.refreshList?.();
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  className="inline-flex items-center gap-1.5 rounded-lg border border-ink-200 bg-white px-2.5 py-1.5 text-xs font-medium text-ink-700 hover:bg-ink-50 dark:border-slate-500/30 dark:bg-ink-900 dark:text-ink-200"
        ...
        {!emailWorkspaceMode ? (
        <motion.div
          className="shrink-0 border-b border-ink-200/70 bg-white/85 shadow-sm backdrop-blur-md dark:border-slate-500/25 dark:bg-[#1c2a40]/55"
        ...
        >
                  ...
                  <div className="flex max-w-full flex-wrap items-center justify-end gap-1.5">
                    ...
                    {copilotEnabled ? (
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 rounded-xl border border-ink-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-ink-700 shadow-sm hover:bg-ink-50 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-100 dark:shadow-none dark:hover:bg-white/10"
                      >
                    ...
                    <button
                      type="button"
                      className="rounded-xl border border-ink-200 bg-white p-2 text-ink-700 shadow-sm hover:bg-ink-50 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-100 dark:shadow-none dark:hover:bg-white/10 xl:hidden"
          ...
          <div
            className={clsx(
              "flex flex-wrap items-center gap-1.5 border-t border-ink-100/80 dark:border-slate-500/25",
              isSplitLayout
        ...
        <div
          ...
          className={clsx(
            ...
            emailWorkspaceMode
              ? "bg-ink-50 px-3 py-4 dark:bg-[#172033] sm:px-5"
        ...
        <motion.div
          className={clsx(
            "w-full min-w-0 shrink-0 border-t border-ink-200/80 dark:border-slate-500/25",
            emailWorkspaceMode
              ? "bg-[#f8fafc] px-4 py-3 dark:bg-[#141c2c]"
              : "bg-ink-50/80 px-3 py-2.5 dark:bg-[#1c2a40]/80 sm:px-4",
          )}
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <form onSubmit={handleSend} className="w-full min-w-0">
            ...
            <div
              className={clsx(
                "composer-shell",
                emailWorkspaceMode && "dark:border-slate-500/30",
              )}
            >
              <div className="flex min-w-0 items-center justify-between gap-3 border-b border-ink-100 px-3 dark:border-slate-500/15">
                ...
                <div className="composer-header-actions pl-1">
                  ...
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-md border border-ink-200 bg-white text-ink-500 hover:bg-ink-50 disabled:opacity-40 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-400 dark:hover:bg-white/10"
              ...
              {(recording || voiceBusy || attachBusy) ? (
                <p className="border-t border-ink-100 px-3 py-2 text-center text-xs text-ink-500 dark:border-slate-500/30 dark:text-ink-400">
                  {attachBusy
        ...
        <div className={clsx("pointer-events-auto absolute right-3 top-28 z-30 hidden xl:block", emailWorkspaceMode && "!hidden")}>
          <div className="flex flex-col gap-1 rounded-2xl border border-ink-200 bg-white/90 p-1 shadow-lg backdrop-blur dark:border-slate-500/25 dark:bg-[#1c2a40]/70 dark:shadow-black/30">
            <button
              type="button"
              onMouseDown={(e) => e.preventDefault()}
      ...
      <aside
        className={clsx(
          "min-h-0 shrink-0 flex-col overflow-hidden border-l border-ink-200/90 bg-white/95 transition-[width] duration-200 ease-out dark:border-slate-500/25 dark:bg-[#1c2a40]/70",
      ...
      >
        <div
          className={clsx(
            "flex shrink-0 items-center border-b border-ink-100 dark:border-slate-500/25",
      ...
      {copilotDesktopOpen ? (
        <aside className="hidden min-h-0 w-[min(100%,min(300px,28vw))] shrink-0 flex-col overflow-hidden border-l border-ink-200/90 bg-white/95 dark:border-slate-500/25 dark:bg-[#1c2a40]/70 xl:flex 2xl:w-[min(100%,340px)]">
          <div className="flex shrink-0 items-center justify-between border-b border-ink-100 px-3 py-2 dark:border-slate-500/25">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
              ...
              <>
                ...
                <div className="mt-2 space-y-2">
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-xl border border-ink-200 bg-white/80 px-3 py-2 text-left text-sm text-ink-800 shadow-sm hover:bg-ink-50 disabled:opacity-60 dark:border-slate-500/25 dark:bg-white/5 dark:text-ink-100 dark:hover:bg-white/10"
                  >
            ...
            {copilotInsights ? (
              <div className="mt-6 space-y-4">
                <div className="rounded-xl border border-ink-200 bg-white p-4 dark:border-slate-500/25 dark:bg-[#24344d]/70">
        ...
        {crmMobileOpen ? (
          ...
          >
            <motion.div
              ...
              className="h-full w-full max-w-md overflow-y-auto border-l border-ink-200 bg-white shadow-2xl dark:border-slate-500/25 dark:bg-[#1c2a40]"
        ...
        {copilotMobileOpen ? (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between border-b border-ink-100 px-4 py-3 dark:border-slate-500/25">
                <p className="text-sm font-semibold text-ink-900 dark:text-ink-50">{t("conversationDetail.copilotTitle")}</p>
                <button
                  type="button"
                  className="rounded-lg p-1.5 text-ink-500 transition-colors hover:bg-ink-100 hover:text-ink-800 dark:text-ink-400 dark:hover:bg-ink-800 dark:hover:text-ink-100"
              ...
              <div className="p-4">
                  ...
                  <>
                    ...
                    <div className="mt-2 space-y-2">
                      <button
                        ...
                        onClick={() => void loadCopilotInsights("summary")}
                        disabled={copilotBusy}
                        className="flex w-full items-center justify-between rounded-xl border border-ink-200 bg-white px-3 py-2 text-left text-sm text-ink-800 shadow-sm hover:bg-ink-50 disabled:opacity-60 dark:border-slate-500/30 dark:bg-ink-800 dark:text-ink-100 dark:hover:bg-ink-700"
                      >
                        <span>{t("conversationDetail.copilotCmdSummarize")}</span>
                        <ChevronRight className="h-4 w-4" />
                      </button>
                ...
                {copilotInsights ? (
                  <div className="mt-6 space-y-4">
                    <div className="rounded-xl border border-ink-200 bg-white p-4 dark:border-slate-500/30 dark:bg-ink-800">
                      <p className="text-xs font-semibold uppercase tracking-wide text-ink-500 dark:text-ink-500">{t("aiInsightsPage.summary")}</p>
                      <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-ink-800 dark:text-ink-100">{copilotInsights.summary}</p>
                    </div>
                  </div>
        ...
        {resolveOpen && (
          ...
          >
            <motion.div
              ...
              animate="show"
              exit="hidden"
              className="max-h-[90vh] w-full max-w-md overflow-auto rounded-2xl bg-white p-6 shadow-xl dark:border dark:border-slate-500/30 dark:bg-ink-900"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <form onSubmit={submitResolve} className="mt-4 space-y-4">
                ...
                {showRemindersFeature && resolveOfferReminder ? (
                  <div className="rounded-xl border border-ink-200 bg-ink-50/80 p-4 dark:border-slate-500/30 dark:bg-ink-800/40">
        ...
        {transferOpen && (
          ...
          >
            <motion.div
              ...
              animate="show"
              exit="hidden"
              className="max-h-[90vh] w-full max-w-lg overflow-auto rounded-2xl bg-white p-6 shadow-xl dark:border dark:border-slate-500/30 dark:bg-ink-900"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start gap-3">
              ...
              <div className="mt-5 space-y-4">
                ...
                <div className="flex justify-end gap-2 border-t border-ink-100 pt-4 dark:border-slate-500/30">
                  <button
                    type="button"
                    disabled={actionLoading}
                  ...
                  <ul className="mt-4 max-h-64 space-y-2 overflow-auto">
                    {orgTags.length === 0 ? (
                      ...
                      orgTags.map((tag) => (
                        <li
                          key={tag.id}
                          className="flex items-center gap-2 rounded-lg border border-ink-100 bg-ink-50/80 px-3 py-2 dark:border-slate-500/30 dark:bg-ink-800/50"
                        >
                          <span
